...
import { deleteBusinessContactApiSchema } from './_schemas/delete-business-contact-schema';
 ...
 */
export type DELETEBusinessContactRequest = {
	businessContactId: string;
};

export async function DELETE(request: Request) {
	// Get request body as json
	const requestBody: DELETEBusinessContactRequest = await request.json();
	...
	try {
		deleteBusinessContactApiSchema.parse(requestBody);
	} catch (error: any) {
		return new Response(
	...
	try {
		await prismaClient().businessContact.delete({
			where: {
